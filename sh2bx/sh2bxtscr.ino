...
int tscr_comm_packuccb(int fsBE, int b6pBE, uint16_t *len)
{
  ...
  *len=0;
  ...
  tscr_comm_pack1((byte*)&g_e_mls,sizeof(g_e_mls),g_wts_commbuf,len);    //2:29-1+9
  ...
  tscr_comm_pack1((byte*)&g_sh1_mlrpm,sizeof(g_sh1_mlrpm),g_wts_commbuf,len);    //2:39
...
{
  g_tscr_takeover=0;
  if((*mls != 0) || (*m2s != 0) || (*rdd != 0)) return(0);
  if((g_tscr_power == 0) && (g_tscr_rudder == 0)) return(0);
  g_tscr_takeover=1;
//  *m1s=(g_tscr_power*(g_sh1_maxmspeed-UCCB_MD_MINSPEED_FWD))/100;

  if(g_tscr_power > 0) {
    *mls=UCCB_MD_MINSPEED_FWD+((long)g_tscr_power*(g_sh1_maxmspeed-UCCB_MD_MINSPEED_FWD))/100;
